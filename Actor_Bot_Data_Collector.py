...
def softmax(x, up_alien_probability, down_alien_probability, left_alien_probability, right_alien_probability):
    mask = []
    if up_alien_probability == -1:
        mask.append(0)
    else:
        mask.append(1)
    if down_alien_probability == -1:
        mask.append(0)
    else:
        mask.append(1)
    if left_alien_probability == -1:
        mask.append(0)
    else:
        mask.append(1)
    if right_alien_probability == -1:
        mask.append(0)
    else:
        mask.append(1)
    mask.append(1)
    e_x = np.exp(x - np.max(x))# * mask
...
def model_control(X_df, W, b, text_label_columns):
    for i in range(len(X_df)):
      ...
      # Convert prediction to label and compare with true label
      predicted_label = one_hot_to_label(y_pred)
...
#print(df)
X_df = df.drop(columns=['Unnamed: 0', 'action_taken', 'actual_crew_pos_x', 'actual_crew_pos_y','next_position_x', 'next_position_y', 'final_result'])
# # text_label_columns = ['on1_direction', 'on2_direction', 'on3_direction', 'on4_direction']
text_label_columns = []
Y = df['action_taken']

# df_sample = df.sample(n = 3000)
# # df = df.drop(columns=['Unnamed: 0.1'])
# X_df = df_sample.drop(columns=['action_taken', 'actual_crew_pos_x', 'actual_crew_pos_y','next_position_x', 'next_position_y', 'final_result'])
# # text_label_columns = ['on1_direction', 'on2_direction', 'on3_direction', 'on4_direction']
# text_label_columns = []
# Y = df_sample['action_taken']
...
output_size = 5  # 5 output neurons for one-hot encoded labels
dropout_rate = 0.3  # Dropout rate, can be adjusted
...
def move_bot1(grid, bot_cell, path, aliens, crewmates, time_steps, df):
  ...
  # Append the new row to the DataFrame
    ...
    if predicted_label == "STAY":
      ...
      new_row = {'time_steps': time_steps, 'bot_position_x': bot_cell.x, 'bot_position_y': bot_cell.y, 'up_alien_prob': up_alien_probability,  'down_alien_prob': down_alien_probability,  'left_alien_prob': left_alien_probability,  'right_alien_prob': right_alien_probability,
            'up_crew_prob': up_crew_probability, 'down_crew_prob': down_crew_probability,'left_crew_prob': left_crew_probability,'right_crew_prob': right_crew_probability,
...
model1_input_df_rescued = pd.DataFrame(columns=['time_steps', 'bot_position_x', 'bot_position_y','actual_crew_pos_x', 'actual_crew_pos_y','up_x', 'up_y', 'down_x', 'down_y', 'left_x', 'left_y', 'right_x', 'right_y','up_alien_prob', 'up_crew_prob', 'down_alien_prob', 'down_crew_prob', 'left_alien_prob', 'left_crew_prob', 'right_alien_prob', 'right_crew_prob', 'max_prob_cell_x', 'max_prob_cell_y', 'next_position_x', 'next_position_y', 'action_taken'])
model1_input_df_caught = pd.DataFrame(columns=['time_steps', 'bot_position_x', 'bot_position_y','actual_crew_pos_x', 'actual_crew_pos_y','up_x', 'up_y', 'down_x', 'down_y', 'left_x', 'left_y', 'right_x', 'right_y','up_alien_prob', 'up_crew_prob', 'down_alien_prob', 'down_crew_prob', 'left_alien_prob', 'left_crew_prob', 'right_alien_prob', 'right_crew_prob', 'max_prob_cell_x', 'max_prob_cell_y', 'next_position_x', 'next_position_y', 'action_taken'])
model1_input_df_combined = pd.DataFrame(columns=['time_steps', 'bot_position_x', 'bot_position_y','actual_crew_pos_x', 'actual_crew_pos_y','up_x', 'up_y', 'down_x', 'down_y', 'left_x', 'left_y', 'right_x', 'right_y','up_alien_prob', 'up_crew_prob', 'down_alien_prob', 'down_crew_prob', 'left_alien_prob', 'left_crew_prob', 'right_alien_prob', 'right_crew_prob', 'max_prob_cell_x', 'max_prob_cell_y', 'next_position_x', 'next_position_y', 'action_taken', 'final_result'])
model1_input_df_timeout = pd.DataFrame(columns=['time_steps', 'bot_position_x', 'bot_position_y','actual_crew_pos_x', 'actual_crew_pos_y','up_x', 'up_y', 'down_x', 'down_y', 'left_x', 'left_y', 'right_x', 'right_y','up_alien_prob', 'up_crew_prob', 'down_alien_prob', 'down_crew_prob', 'left_alien_prob', 'left_crew_prob', 'right_alien_prob', 'right_crew_prob', 'max_prob_cell_x', 'max_prob_cell_y', 'next_position_x', 'next_position_y', 'action_taken', 'final_result'])
...
while len(model1_input_df_rescued) < 20000 or len(model1_input_df_caught) < 20000:
  for row in grid:
    for cell in row:
      if cell.open:
        cell.value = 1

  model1_input_df = pd.DataFrame(columns=['time_steps', 'bot_position_x', 'bot_position_y','actual_crew_pos_x', 'actual_crew_pos_y','up_x', 'up_y', 'down_x', 'down_y', 'left_x', 'left_y', 'right_x', 'right_y','up_alien_prob', 'up_crew_prob', 'down_alien_prob', 'down_crew_prob', 'left_alien_prob', 'left_crew_prob', 'right_alien_prob', 'right_crew_prob', 'max_prob_cell_x', 'max_prob_cell_y', 'next_position_x', 'next_position_y', 'action_taken'])
  ...
  model1_input_df_rescued.to_csv('Balanced_Actor_rescue_temp.csv')

  print("CAUGHT DF SO FAR")
  model1_input_df_caught['final_result']='CAUGHT'
  model1_input_df_timeout['final_result']='CAUGHT'
  print(model1_input_df_caught)
  model1_input_df_caught.to_csv('Balanced_Actor_caught_temp.csv')
  
  print("TIME OUT SO FAR")
  print(len(model1_input_df_timeout['final_result']))
  model1_input_df_timeout.to_csv('Balanced_Actor_timeout_temp.csv')

  model1_input_df_combined = pd.concat([model1_input_df_combined, model1_input_df_rescued, model1_input_df_caught, model1_input_df_timeout], ignore_index=True)

  model1_input_df_combined.to_csv('Balanced_Actor_combined-temp.csv')


print("RESCUED DF")
# model1_input_df_rescued['final_result']='RESCUED'
print(model1_input_df_rescued)
model1_input_df_rescued.to_csv('Balanced_Actor_rescued.csv')
...
# model1_input_df_caught['final_result']='CAUGHT'
print(model1_input_df_caught)
model1_input_df_caught.to_csv('Balanced_Actor_caught.csv')

print("TIMEOUT DF")
# model1_input_df_caught['final_result']='CAUGHT'
print(model1_input_df_timeout)
model1_input_df_timeout.to_csv('Balanced_Actor_timeout.csv')

# model1_input_df_combined = pd.concat([model1_input_df_combined, model1_input_df_caught])

model1_input_df_combined.to_csv('Balanced_Actor_combined.csv')
